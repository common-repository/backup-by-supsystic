...
class templatesBup extends moduleBup {
	...
	public function loadBootstrapSimple() {
		static $loaded = false;
		if(!$loaded) {
			frameBup::_()->addStyle('bootstrap-simple', BUP_CSS_PATH. 'bootstrap-simple.css');
			$loaded = true;
		}
	}
	public function loadGoogleFont( $font ) {
		...
		if(!isset($loaded[ $font ])) {
			frameBup::_()->addStyle('google.font.'. str_replace(array(' '), '-', $font), 'https://fonts.googleapis.com/css?family='. urlencode($font));
			$loaded[ $font ] = 1;
		}
	}
}
