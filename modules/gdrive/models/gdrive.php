...
class gdriveModelBup extends modelBup {
    ...
    public function getCredential($key) {
        ...
        $value = frameBup::_()->getTable('options')->get('value', array('code' => $key), '', 'one');
     ...
     */
    public function saveCredential($key, $value)
    {
        $key = ($key == 'clientId' ? self::CLIENT_ID_INDEX : self::CLIENT_SECRET_INDEX);
        $current = frameBup::_()->getTable('options')->get('value', array('code' => $key), '', 'one');
        if(is_null($current)) {
            frameBup::_()->getTable('options')->insert(array('code' => $key, 'value' => $value));
        } else {
            if($current != $value) {
                frameBup::_()->getTable('options')->update(array('value' => $value), array('code' => $key));
            }
        }
    }
	...
	public function getClient() {
        ...
        $client->setRedirectUri(admin_url('admin.php?page=' . $slug));
	...
	public function authenticate() {
        ...
        if(!isset($request['code']) OR empty($request['code'])) {
    ...
    public function getAuthenticationURL() {
        ...
        $slug = frameBup::_()->getModule('adminmenu')->getView()->getFile();
        $redirectURI = 'admin.php?page=' . $slug;

        $client->setClientId($credentials['clientId']);
        $client->setClientSecret($credentials['clientSecret']);
        $client->setRedirectUri(admin_url($redirectURI));
        $client->setScopes($this->_scopes);
