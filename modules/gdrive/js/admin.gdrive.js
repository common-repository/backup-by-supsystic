jQuery(document).ready(function(){
	...
	j('#bupGDriveCredentials').on('click', function(event) {
	...
	authenticate: function(form) {
		jQuery(form).sendFormBup({
			...
			data: {
				'reqType': 'ajax',
				'page':    'gdrive',
				'action':  'beginAuthenticationAction',
				'client_id': jQuery('input[name="gdrive_client_id"]').val(),
				'client_secret': jQuery('input[name="gdrive_client_secret"]').val()
			},
			onSuccess: function(response) {
				if(response.error === false && response.data && response.data.authUrl) {
					toeRedirect(response.data.authUrl);
				}
			}
		});
	},
