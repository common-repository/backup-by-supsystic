...
class promo_supsysticBup extends moduleBup {
	...
	public function __construct($d) {
		...
		dispatcherBup::addAction('discountMsg', array($this, 'getDiscountMsg'));
	}
	...
	public function registerOverviewTab($tabs) {
		...
		$tabs['featured-plugins'] = array(
			'title' => __('Featured Plugins', BUP_LANG_CODE), 'content' => array($this, 'showFeaturedPluginsPage'), 'faIcon' => 'fa-heart', 'sort_order' => 99,
		);
		return $tabs;
	}
	...
	public function getDiscountMsg() {
		if($this->isPro()
			&& frameBup::_()->getModule('options')->getActiveTab() == 'license'
			&& frameBup::_()->getModule('license')
			&& frameBup::_()->getModule('license')->getModel()->isActive()
		) {
	...
	public function getDiscountBuyUrl() {
		$license = frameBup::_()->getModule('license')->getModel()->getCredentials();
		$license['key'] = md5($license['key']);
		$license = urlencode(base64_encode(implode('|', $license)));
		$plugin_code = 'backup_by_supsystic_pro';
