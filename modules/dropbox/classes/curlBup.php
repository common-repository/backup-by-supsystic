...
class curlBup {
     ...
     */
    public function setPostRequest(array $fields = array(), $encode)
    {
        $this->setOption('POST', true);
        if($encode){
        	$args = json_encode($fields);
			$this->setOption('PostFields', $args);
		}else{
     ...
     * @return curlBup
     */
    public static function createPostRequest($url, array $fields = array(), $encode = false)
    {
        $dropboxCurl = new self($url);
		$dropboxCurl->setPostRequest($fields, $encode);
