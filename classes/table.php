...
abstract class tableBup {
            ...
            if($id) {
                ...
                foreach($data as $field) {
                    ...
                    foreach($field as $k => $v) {
                        if(isset($this->_fields[$k])) {
                            $row[$k] = toeCreateObjBup('fieldBup', array(
                                    $this->_fields[$k]->name,
                                    $this->_fields[$k]->html,
                                    $this->_fields[$k]->type,
    ...
    public function getTable($transform = false) {
        if($transform)
            return dbBup::prepareQuery($this->_table);
        else
            return $this->_table;
    }
    ...
    public function get($fields = '*', $where = '', $tables = '', $return = 'all') {
        ...
        return dbBup::get($query, $return);
    }
    public function store($data, $method = 'INSERT', $where = '') {
		$this->_clearErrors();
        $method = strtoupper($method);
		if($this->_escape) {
			$data = dbBup::escape($data);
		}
        $query = '';
        switch($method) {
            case 'INSERT':
        ...
        if(is_array($data) && !empty($data)) {
            foreach($data as $k => $v) {
                if(array_key_exists($k, $this->_fields) || $k == $this->_id) {
                    ...
                    if($validate) {
                        if ($this->_fields[$k] && is_object($this->_fields[$k])) {
    ...
    public function exists($value, $field = '') {
        if(!$field)
            $field = $this->_id;
        return dbBup::get('SELECT '. $this->_id. ' FROM '. $this->_table. ' WHERE '. $field. ' = "'. $value. '"', 'one');
    }
            ...
            switch($f->type) {
                case 'date':
                    if($d[$key] == '0000-00-00' || empty($d[$key]))
                        ...
                        $d[$key] = date(BUP_DATE_FORMAT, dbBup::dateToTime($d[$key]));
                    }
                    break;
                case 'int':
                case 'tinyint':
