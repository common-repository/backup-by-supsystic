...
class modInstallerBup {
	...
	static private function _getExtendModules($locations) {
		...
		if(!empty($modulesList)) {
			foreach($modulesList as $mod) {
				$modData = $isExtendModPath ? $mod : utilsBup::xmlNodeAttrsToArr($mod);
				array_push($modules, $modData);
			}
			if(empty($modules))
				errorsBup::push(__('No modules were found in installation file', BUP_LANG_CODE), errorsBup::MOD_INSTALL);
			else
				return $modules;
		} else
			errorsBup::push(__('No installation file were found', BUP_LANG_CODE), errorsBup::MOD_INSTALL);
		return false;
	}
    /**
    ...
    static public function check($extPlugName = '') {
		...
		if($modules = self::_getExtendModules($locations)) {
			foreach($modules as $m) {
				if(!empty($m)) {
					//If module Exists - just activate it, we can't check this using frameBup::moduleExists because this will not work for multy-site WP
					if(frameBup::_()->getTable('modules')->exists($m['code'], 'code') /*frameBup::_()->moduleExists($m['code'])*/) {
						self::activate($m);
					} else {                                           //  if not - install it
						if(!self::install($m, $locations['plugDir'])) {
							errorsBup::push(sprintf(__('Install %s failed'), $m['code']), errorsBup::MOD_INSTALL);
						}
					}
				}
			}
    ...
    static public function deactivate() {
		...
		if($modules = self::_getExtendModules($locations)) {
			foreach($modules as $m) {
				if(frameBup::_()->moduleActive($m['code'])) { //If module is active - then deacivate it
					if(frameBup::_()->getModule('options')->getModel('modules')->put(array(
						'id' => frameBup::_()->getModule($m['code'])->getID(),
						'active' => 0,
					))->error) {
    ...
    static public function activate($modDataArr) {
		if(!empty($modDataArr['code']) && !frameBup::_()->moduleActive($modDataArr['code'])) { //If module is not active - then acivate it
			if(frameBup::_()->getModule('options')->getModel('modules')->put(array(
    ...
    static public function uninstall() {
		...
		if($modules = self::_getExtendModules($locations)) {
			foreach($modules as $m) {
				...
				frameBup::_()->getModule('options')->getModel('modules')->delete(array('code' => $m['code']));
				utilsBup::deleteDir(BUP_MODULES_DIR. $m['code']);
			}
		}
    }
