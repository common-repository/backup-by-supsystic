...
jQuery(document).ready(function($){
	...
	if(typeof(bupActiveTab) != 'undefined' && bupActiveTab != 'bupMainOptions' && jQuery('#toplevel_page_supsystic-backup').hasClass('wp-has-current-submenu')) {
		...
		subMenus.removeClass('current').each(function(){
			if(jQuery(this).find('a[href$="&tab=' + bupActiveTab + '"]').length) {
...
/*-----*/
	...
	changeBgTypeOptionBup();
	 ...
	 jQuery('.bupOptTip').on('mouseover',function(event){
    ...
    jQuery('.bupOptTip').on('mouseout',function(){
		toeOptTimeoutHideDescriptionBup();
        return false;
    });
	jQuery('#bupOptDescription').on('mouseover',function(e){
		jQuery(this).attr('toeFixTip', 1);
		return false;
    });
	jQuery('#bupOptDescription').on('mouseout',function(e){
		toeOptTimeoutHideDescriptionBup();
		return false;
    });
...
function changeBgTypeOptionBup() {
	...
	if(jQuery('#bupAdminTemplateOptionsForm [name="opt_values[bg_type]"]:checked').length)
